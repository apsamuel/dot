...
function dot::terminal::compile-info() {
    infocmp -x xterm-256color
    printf '\t%s\n' 'ncv@,'
} >/tmp/t && tic -x /tmp/t

# fzf
function dot::terminal::configure-fzf() {
    ...
    if  command -v fzf  >/dev/null 2>&1; then
        ...
        . "$HOMEBREW_PREFIX"/Cellar/fzf/"${fzf_version}"/shell/completion.zsh || dot::static::logging::warn "error loading fzf completion"

        # translate bind to bindkey
        . "$HOMEBREW_PREFIX"/Cellar/fzf/"${fzf_version}"/shell/key-bindings.zsh || dot::static::logging::warn "error loading fzf key bindings"
