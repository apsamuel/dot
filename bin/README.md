...
| `changelog.sh` | Bash | Generates a formatted git changelog for the `dot` repo from conventional commits. |
| `mask.sh` | ZSH | Secret masking, detection, ZLE integration, and git pre-commit hook installer. Four modes: `-m` mask text, `-c` check for secrets, `-Z` print sourceable ZLE hooks, `-G <path>` install a git pre-commit hook. Reads secrets from a flat JSON file (`-f`). |
