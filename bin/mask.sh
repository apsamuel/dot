...
#
# mask.sh — secret masking and detection utility
#
...
#           Usage:  source <(mask.sh -Z)
#   hook    Install a git pre-commit hook that runs check mode on staged content,
#           preventing secrets from reaching a repository.
#           Usage:  mask.sh -G <repo_path>
#
#% description: mask, detect, and prevent secrets in text, terminal, and git
#% usage:
#%   mask.sh [options] [string ...]           mask / check positional args
#%   echo "text" | mask.sh [options]          mask / check stdin
#%   mask.sh -i <file> [options]              mask / check a file
#%   source <(mask.sh -Z)                     activate ZLE + history hooks
#%   mask.sh -G <repo_path>                   install git pre-commit hook
#%
...
#%
#% examples:
#%   mask.sh "my api key is ghp_xxxx"
#%   env | mask.sh
#%   mask.sh -i /var/log/app.log > /tmp/safe_log.txt
#%   mask.sh -c -i report.txt && echo "file is clean"
#%   mask.sh -G .                         # install pre-commit hook in cwd
#%   source <(mask.sh -Z)                 # add hooks to current zsh session
...
install_git_hook() {
    ...
    if [[ -f "$hook_file" ]] && grep -q "mask.sh secret check" "$hook_file" 2>/dev/null; then
        warn "mask.sh pre-commit block already present in ${hook_file} — skipping."
        return 0
    fi
...
print_zle_setup() {
    ...
    cat <<ZSHEOF
# ── mask.sh ZLE integration ────────────────────────────────────────────────────
# Generated by: ${mask_script} -Z
# Source in .zshrc with:  source <(mask.sh -Z)
...
function maskSecrets() {
    ...
    _mask_refresh_cache
    local sw="\${MASK_SAFE_WORD:-\$_MASK_SAFE_WORD}"
    printf '\e[32m✔  mask.sh hooks active  (Alt-m masks buffer · safe-word: %s)\e[0m\n' "\$sw" >&2
}
...
function unmaskSecrets() {
    ...
    printf '\e[33m⚠  mask.sh hooks deactivated\e[0m\n' >&2
}

# Auto-activate on source
maskSecrets
# ── end mask.sh ZLE integration ───────────────────────────────────────────────
ZSHEOF
}
